...
describe('Trade Formatter', () => {
  describe('convertToIntelInvestFormat', () => {
    it('should correctly format a buy trade', () => {
      ...
      expect(dealInParts[0]).to.equal(SERVICE_CONSTANTS.TRADE_TYPES.SHARE_BUY);
      ...
      expect(dealOutParts[0]).to.equal(SERVICE_CONSTANTS.TRADE_TYPES.SHARE_OUT);
    ...
    it('should correctly format a sell trade', () => {
      ...
      const trade = [
        {
          side: SERVICE_CONSTANTS.SELLING_TYPE,
          transactionTime: timestamp.toString(),
          cashFlow: '0.5',
          currency: 'ETH',
          tradePrice: '1500',
      ...
      expect(dealInParts[0]).to.equal(SERVICE_CONSTANTS.TRADE_TYPES.SHARE_SELL);
      expect(dealInParts[1]).to.equal(formattedDate);
      ...
      expect(dealOutParts[0]).to.equal(SERVICE_CONSTANTS.TRADE_TYPES.SHARE_IN);
      expect(dealOutParts[2]).to.equal('USDT:USDT');
